...
2. Extract patches from an image, based on those coordinates

So that the user has the option to use the same set of coordinates across all image
or generate a new set of coordinates for each image."""
...
method='non-overlapping'):
    ...
    method: 'random' or 'nonoverlapping'.
        'random': If patches are generated fully at random, then they can overlap.
...
def extract_patches_3d(image, coords, patch_len, ds_factor):
    ...
    For the moment, all 3 colour channels are extracted for each set of coordinates...
    I'll decide what to do about colour channels later.
